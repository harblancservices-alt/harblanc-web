...
/**
 * Executable definition of Performance's range resolver — the new preset
...
describe("resolvePerformanceView — fixed presets", () => {
  it("today is [today, tomorrow) with prevRange = yesterday", () => {
    const v = resolvePerformanceView({ range: "today" }, NOW);
    expect(v.mode).toBe("range");
    expect(v.range).toEqual({ start: "2026-08-08", end: "2026-08-09" });
    expect(v.prevRange).toEqual({ start: "2026-08-07", end: "2026-08-08" });
    expect(v.granularity).toBe("day");
  });

  it("yesterday is [yesterday, today) with prevRange = day before", () => {
    const v = resolvePerformanceView({ range: "yesterday" }, NOW);
    expect(v.range).toEqual({ start: "2026-08-07", end: "2026-08-08" });
    expect(v.prevRange).toEqual({ start: "2026-08-06", end: "2026-08-07" });
  });

  it("this_week is Sunday-anchored (Calendar's own week-key convention)", () => {
...
describe("resolvePerformanceView — month (unchanged) and custom", () => {
  it("defaults to the current Central month with no params", () => {
  ...
  it("?month= browses an explicit month with a calendar-month prevRange", () => {
  ...
  it("custom precedence wins over year/month/range when all are present", () => {
    const v = resolvePerformanceView({ from: "2026-08-01", to: "2026-08-02", year: "2025", month: "2026-01", range: "today" }, NOW);
    expect(v.mode).toBe("custom");
  });

  it("a wide custom range falls back to weekly, then monthly granularity", () => {
