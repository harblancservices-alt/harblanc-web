...
/** Dependency-free trailing-months Net trend — flexbox bars off a zero
 ...
 * size. Negative-net months render below the baseline in --bad. */
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-end gap-2" style={{ height: CHART_HEIGHT }}>
        {points.map((p) => {
          const h = Math.max(2, Math.round((Math.abs(p.net) / maxAbs) * (CHART_HEIGHT - 24)));
          const positive = p.net >= 0;
          return (
            <div key={p.label} className="flex flex-1 flex-col items-center justify-end gap-1" title={`${p.label}: ${formatMoney(p.net)}`}>
              <span className="text-[11px] font-medium tabular-nums text-fg-muted">{formatMoney(p.net)}</span>
              <div
