...
/**
 * BOL finger-signature flow — ported from V1's BolSigner.tsx (place → sign →
 * confirm → save) essentially unchanged per the phase brief ("largest
 * capability gap... the flow being ported already works well"). Only the
 * Button import/variant names and the action import point at /tms-v2; the
 * placement/compositing math and signBolRole call are identical.
 */

const SIG_WIDTH_FRAC = 0.34;
const DEFAULT_SIG_ASPECT = 0.34;
const IMG_MAX_DIM = 2400;
...
function isPdfDoc(doc: LoadDocumentItem): boolean {
  return (doc.mime ?? "").includes("pdf") || /\.pdf$/i.test(doc.name);
}

const ROLE_LABEL: Record<BolRole, string> = { receiver: "Receiver", carrier: "Carrier" };
...
export function BolSigner({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setErr(e instanceof Error ? e.message : "Could not open the BOL.");
      }
    })();
    return () => {
      cancelled = true;
  ...
  const handleSignDone = useCallback((sig: Signature, name: string, date: string) => {
    setSignature(sig);
    setPrintName(name);
    setDateStr(date);
    setErr(null);
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isPdf) {
        ...
        const [cx, cy] = vp1.convertToPdfPoint(placement.fx * vp1.width, placement.fy * vp1.height) as [number, number];
        payload = {
        ...
        payload = {
          ...
          printName,
          dateStr,
          placement: { kind: "image", fx: placement.fx, fy: placement.fy, widthFrac: SIG_WIDTH_FRAC, aspect: signature.aspect },
        };
      }
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Sign bill of lading" className="fixed inset-0 z-50 flex flex-col bg-black/85">
      <div className="flex items-center justify-between gap-3 bg-bar px-4 py-2.5">
        ...
        <Button type="button" variant="secondary" size="sm" onClick={onClose} disabled={step === "saving"}>
          Cancel
        </Button>
      </div>
      ...
      <div className="no-scrollbar min-h-0 flex-1 overscroll-contain overflow-y-auto px-2 py-3">
        {step === "loading" ? (
          ...
          <p className="mt-10 text-center text-[13px] font-semibold text-white/80">Saving signed BOL…</p>
        ) : (
          <>
      ...
      {step === "place" ? (
        <div className="flex items-center justify-between gap-3 border-t border-white/10 bg-bar px-4 py-3">
          <span className="text-[12px] text-bar-fg/70">{placement ? "Drag to position" : "Tap the BOL to place"}</span>
          {signature ? (
            <div className="flex items-center gap-2">
              <Button type="button" variant="secondary" size="sm" onClick={() => setStep("sign")}>
                Re-sign
              </Button>
              <Button type="button" variant="primary" size="sm" onClick={() => setStep("confirm")} disabled={!placement}>
                Preview →
              </Button>
            </div>
          ) : (
            <Button type="button" variant="primary" size="sm" onClick={() => setStep("sign")} disabled={!placement}>
              Sign here →
            </Button>
          )}
        </div>
      ...
      {step === "confirm" ? (
        <div className="flex items-center justify-between gap-3 border-t border-white/10 bg-bar px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <Button type="button" variant="secondary" size="sm" onClick={() => setStep("place")}>
              Redo
            </Button>
...
function PlacementSurface({
  ...
  return (
    ...
    >
      ...
      <img src={src} alt={alt} className="block w-full rounded-sm bg-white" draggable={false} />
      {active ? (
        <PlacementBox fx={fx} fy={fy} wPct={wPct} hPct={hPct} signatureUrl={signatureUrl} label={signatureUrl ? label : ""} />
      ) : null}
    </div>
  );
}
...
function computePadView(): PadView {
  if (typeof window === "undefined") return { rot: 0, W: 0, H: 0, portrait: false };
...
function SignaturePad({
  ...
  useEffect(() => {
    const onResize = () =>
      setView((prev) => {
        ...
        return next.rot === prev.rot && next.W === prev.W && next.H === prev.H ? prev : next;
      });
    onResize();
  ...
  function posOf(e: React.PointerEvent) {
    ...
    if (view.rot === 90) return { x: dy + Wc / 2, y: -dx + Hc / 2 };
  ...
  async function done() {
    ...
    onDone({ png: sig.png, dataUrl: sig.dataUrl, aspect: sig.h / sig.w }, printName, dateStr);
  }

  return (
    <div role="dialog" aria-modal="true" aria-label="Signature pad" className="fixed inset-0 z-[60] touch-none overflow-hidden bg-neutral-900">
      <div
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-1.5 p-3">
          <p className="text-center text-[12px] font-semibold text-white/80">Sign with your finger above the line</p>
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="flex flex-col gap-1">
              <span className="font-mono text-[10px] font-bold uppercase tracking-[0.1em] text-white/60">Print name (optional)</span>
              <input
                type="text"
                value={printName}
            ...
            <label className="flex flex-col gap-1">
              <span className="font-mono text-[10px] font-bold uppercase tracking-[0.1em] text-white/60">Date</span>
              <input
                type="text"
                value={dateStr}
        ...
        <div className="grid grid-cols-2 gap-3 border-t border-white/10 bg-bar px-4 py-2.5">
          <Button type="button" variant="secondary" onClick={clear} disabled={!hasInk || busy} className="w-full">
            Clear
          </Button>
          <Button type="button" variant="primary" onClick={done} disabled={!hasInk || busy} aria-busy={busy} className="w-full">
            {busy ? "Working…" : "Done"}
          </Button>
        </div>
      </div>
...
async function extractSignaturePng(canvas: HTMLCanvasElement): Promise<{ png: Uint8Array; dataUrl: string; w: number; h: number } | null> {
  const ctx = canvas.getContext("2d");
  if (!ctx) return null;
  ...
  if (!blob) return null;
  return { png: new Uint8Array(await blob.arrayBuffer()), dataUrl: out.toDataURL("image/png"), w: cw, h: ch };
}
