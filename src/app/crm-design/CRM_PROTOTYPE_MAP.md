...
| Company detail | `/crm-design/companies/[id]` | Full record: stage, details, contacts, activity, documents, tasks | Everyone with visibility | Move stage, log activity, add contact, generate document, complete a task | Companies list, any company link elsewhere | Contact detail, Admin → Activity Log (deep-link target) |
...
| Admin Account (Overview) | `/crm-design/admin` | Landing summary + accurate description of each Admin tab | Owner/Admin | Jump to any Admin tab via stat tiles | Sidebar → Admin Account | Accounts, Activity Log, Documents, Organization |
...
| Admin → Documents | `/crm-design/admin/documents` | The org's 2 blank master templates (Rate Confirmation, Bill of Lading) | Owner/Admin | Preview a template | Admin tab bar → Documents | Company detail's Documents tab (where documents are actually generated) |
