...
| Dashboard | `/crm-design` | Daily command center — what's overdue, what's next, **your own** recent activity | Everyone | Complete a task, open a company from NBA/activity, "Add company" | Sidebar → Dashboard | Companies, Tasks, Prospects |
...
| Company detail | `/crm-design/companies/[id]` | Full record: stage, details, contacts, activity, documents, tasks, **Intelligence** (locations/observed freight/lanes from BOL Center AND/OR OTR — empty until an admin releases something) | Everyone with visibility | Move stage, log activity, add contact, generate document, complete a task | Companies list, any company link elsewhere | Contact detail, Admin → Activity Log (deep-link target), Admin → BOL detail / OTR (Intelligence tab's "Sources," owner/admin only) |
...
| **Prospects** | `/crm-design/prospects` | Sales-facing card view of every company released from OTR or BOL Center — one card per company, source tagged, grouped across multiple releases | Everyone | Open a company | Sidebar → Prospects | Company detail |
...
| Calendar *(unlinked from nav)* | `/crm-design/calendar` | Month view of task due dates + follow-ups | Everyone (deep link only) | Select a day to see its events | Dashboard's Calendar quick-link | Company detail |
| Activity Feed *(unlinked from nav)* | `/crm-design/activities` | Org-wide SALES activity (calls/notes/emails/stage changes/documents) | Everyone (deep link only) | Search, filter by type, jump to company | Dashboard's "View org-wide" link | Company detail. **Not** the admin audit log — see Admin → Activity Log below |
...
| Admin Account (Overview) | `/crm-design/admin` | Landing summary + accurate description of each Admin tab | Owner/Admin | Jump to any Admin tab via stat tiles | Sidebar → Admin Account | BOL Center, OTR, Accounts, Activity Log, Documents, Organization |
| Admin → BOL Center (Inbox) | `/crm-design/admin/bol-center` | Intake queue for every uploaded BOL — status funnel (New→Needs Review→AI Extracted→Researching→Ready for Approval→Approved→Released / Rejected / Archived) | Owner/Admin | Search, filter by status, "Upload BOL" | Admin tab bar → BOL Center (2nd slot, badge = needs-attention count) | BOL detail |
| Admin → BOL detail | `/crm-design/admin/bol-center/[id]` | One BOL's full review workspace — persistent original photo (left) + tabbed review (right) | Owner/Admin | Run AI extraction, correct fields, confirm/reject customer match, review locations, verify contacts, write research notes, Approve/Reject, pick release checklist + Release to Sales (record stays here as "Released," document never removed) | BOL Center Inbox row | Company detail + Prospects (only after release), another BOL (via duplicate-hint "Compare" or BOL History) |
| **Admin → OTR** | `/crm-design/admin/otr` | Review queue for document-less "Dispatch \<company\>" verbal prospects — a smaller, separate funnel from BOL Center on purpose | Owner/Admin | Start Research, write notes, set sales relevance, Mark Ready for Approval, Release to Prospects, Reject | Admin tab bar → OTR (3rd slot, badge = needs-attention count) | Company detail + Prospects (only after release) |
