...
/**
 * Notes on the company profile, split into two unmistakably different
 * groups so a machine-written research brief never reads as a teammate's
 * note (or vice-versa): "AI Research" (is_ai=true, read-only besides
 * delete, amber-accented) and "Team notes" (is_ai=false, the normal
 * author + pin + compose flow). Adding a note logs a timeline activity via
 * the server action; new team notes always write is_ai=false — AI briefs
 * are inserted externally, never from this composer.
 */
export function NotesSection({
  ...
  return (
    <>
      {aiNotes.length > 0 && <AiResearchBlock accountId={accountId} notes={aiNotes} />}
      <TeamNotesBlock accountId={accountId} notes={teamNotes} />
    </>
  );
}
...
function AiResearchBlock({
  ...
  function remove(note: CrmNote) {
    if (!window.confirm("Delete this AI research brief? This can't be undone.")) return;
    setBusyId(note.id);
  ...
  return (
    <Card>
      ...
      <ul className="divide-y divide-line-strong">
        {notes.map((n) => (
          <li key={n.id} className="border-l-4 border-l-warn bg-warn-bg/40 px-5 py-3.5">
