  ...
  return (
    <div className={PAGE_CONTAINER}>
      <div className="space-y-4">
        <div className="rounded-lg border border-line-strong bg-card p-4 sm:p-5">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-16 animate-pulse rounded-lg border border-line-strong bg-inset" />
          ))}
        </div>

        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="overflow-hidden rounded-lg border border-line-strong bg-card">
