...
import { AccountingView, type AccountingData } from "../accounting/AccountingView";

/**
 * Operations · Accounting tab.
 *
 * The former /admin/accounting page, moved into the Operations hub. Loader
 * (loadAccounting) is unchanged:
 *
 ...
 * processing fees, payouts to the bank, and current balance — are pulled live
 * from the Stripe API on each load and degrade gracefully if the secret key
 * isn't configured.
 *
 * AccountingView now renders embedded (no standalone PageHeader / page frame);
 * the hub supplies the header and the Open-in-Stripe action lives in the view's
 * own toolbar.
 */
...
export async function AccountingPanel() {
