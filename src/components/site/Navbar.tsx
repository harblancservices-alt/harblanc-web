  ...
  return (
    <header className="sticky top-0 z-40 border-b border-neutral-800 bg-neutral-950">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        >
          <BrandLogo variant="inverted" priority className="h-9 w-auto md:hidden" />
          <BrandLogo variant="inverted" priority className="hidden h-9 w-auto md:block" />
        </Link>
        ...
        <nav className="hidden md:flex md:items-center md:gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-xs font-bold uppercase tracking-[0.12em] text-neutral-400 transition-colors hover:text-white"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="hidden md:block">
          <Link
            href="/quote"
            className="inline-flex items-center bg-red-600 px-4 py-2 text-xs font-bold uppercase tracking-[0.12em] text-white transition-colors hover:bg-red-500"
          >
            Request a Quote
          </Link>
        </div>
