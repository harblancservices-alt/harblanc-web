...
import { renderFinalizedQuotePdfBuffer } from "@/lib/pdf/renderFinalizedQuotePdf";
import type { FinalizedQuotePdfData } from "@/lib/pdf/FinalizedQuotePDF";
import type { ViewerError } from "@/lib/domain/revenue-estimate";

/**
 * Finalized Quote (rate confirmation) — the second stage of the revenue
 * pipeline (Estimate → Quote → BOL → Payment). Shared by both /admin and
 ...
 * wrapper files: src/app/admin/(authed)/quotes/finalized-quote-actions.ts
 * and src/actions/tms-v2/pipeline.ts.
 *
 * Deliberately EXCLUDES resendFinalizedQuote — stays in admin's
 * finalized-quote-actions.ts unchanged; /tms-v2 doesn't call it
 * (confirmed: pipeline.ts is tms-v2's only entry point into the four
 * quotes/*.ts admin files).
 *
 * Lifecycle:
 ...
 *
 *   4. sendFinalizedQuote(finalizedQuoteId)
 ...
 *          if scope changes.
 *
 * sendFinalizedQuote auto-advances lead_status to `awaiting_payment`
 * from any pre-payment funnel state. The subsequent `awaiting_payment`
 * -> `ready_to_dispatch` transition is driven by
 * revenue-payment.ts::recordPayment.
 *
...
// ─────────────────────────────────────────────────────────────────────────
//  1. Generate Finalized Quote Draft — entry point.
//
...
//     reviewed are exactly the bytes the customer receives.
// ─────────────────────────────────────────────────────────────────────────
...
export async function buildFinalizedQuotePreview(
  ...
  // Pull the row's confirmation_token (auto-generated at row insert via
  // the schema default) so we can stitch the customer-facing confirm
  // URL into the rendered email. The URL is baked into preview_html at
  // this moment and shipped verbatim at send time, preserving
  // preview/send byte parity.
  ...
  // buildEstimatePreview. Building the preview returns the rendered email
  // to the client modal; a page revalidate mid-edit raced with the Load
  // Details auto-save and wiped freshly-typed stops & addresses. The
  // draft is persisted above; the page refreshes its state on the next
  // nav or on Send. Callers do their own revalidatePath.
...
export async function sendFinalizedQuote(
  ...
  // Auto-advance lead status from any pre-payment funnel state to
  // awaiting_payment. Mirror of the sendEstimate auto-advance pattern:
  // read current status, only advance from the expected source states,
 ...
 * because that module is email-only; this is a shared domain function.
 */
...
// ─────────────────────────────────────────────────────────────────────────
//  Read-only viewer routes — "View email" / "View PDF" on a sent finalized
//  quote. Shared by /admin's and /tms-v2's finalized-quote-email/
//  finalized-quote-pdf route handlers (retirement-readiness Objective 1C).
//  No auth here — each wrapper does its own auth check before calling.
// ─────────────────────────────────────────────────────────────────────────
...
export async function renderFinalizedQuoteEmailView(
  quoteRequestId: string,
  finalizedQuoteId: string,
  ...
  const { data: row, error } = await sb
    ...
    .eq("id", finalizedQuoteId)
    .eq("quote_request_id", quoteRequestId)
    .maybeSingle<FqEmailViewRow>();
  if (error) {
    return { ok: false, status: 500, error: `Finalized quote lookup failed: ${error.message}` };
  }
  if (!row || !row.preview_html) {
    return { ok: false, status: 404, error: "No email preview stored for this finalized quote." };
  }

  const html = renderPreviewEmailWrapper({
    title: "Finalized quote email",
...
export async function renderFinalizedQuotePdf(
  quoteRequestId: string,
  finalizedQuoteId: string,
  ...
  const { data: fq, error: fqErr } = await sb
    ...
    .eq("id", finalizedQuoteId)
    .eq("quote_request_id", quoteRequestId)
    .maybeSingle<FqPdfViewRow>();

  if (fqErr) {
    return { ok: false, status: 500, error: `Finalized quote lookup failed: ${fqErr.message}` };
  }
  if (!fq) {
    return { ok: false, status: 404, error: "Finalized quote not found." };
  }
  ...
  const { data: lead } = await sb
    ...
    .maybeSingle<FqPdfLeadRow>();
  if (!lead) {
    return { ok: false, status: 404, error: "Lead not found." };
  }

  const linehaul = pdfNum(fq.linehaul);
  const totalAmount = pdfNum(fq.total_amount);
  if (linehaul === null || totalAmount === null) {
    return {
      ok: false,
      status: 422,
      error: "Rate breakdown is incomplete on this finalized quote. Open the workspace and build a preview first.",
    };
  }
  ...
  const data: FinalizedQuotePdfData = {
    ...
    freight: {
      ...
      lengthIn: pdfNum(fq.length_in),
      widthIn: pdfNum(fq.width_in),
      heightIn: pdfNum(fq.height_in),
      weightLbs: pdfNum(fq.exact_weight_lbs),
    ...
    rate: {
      linehaul,
      fuelSurcharge: pdfNum(fq.fuel_surcharge),
      permitsFee: pdfNum(fq.permits_fee),
      accessorials: pdfParseAccessorials(fq.accessorials),
      totalAmount,
    },
  ...
  try {
    const buffer = await renderFinalizedQuotePdfBuffer(data);
    return { ok: true, buffer, filename: `${fq.finalized_quote_number}.pdf` };
  } catch (e) {
    console.error("[renderFinalizedQuotePdf] render failed", {
      finalizedQuoteId: fq.id,
      message: e instanceof Error ? e.message : "unknown",
    });
    return { ok: false, status: 500, error: "Could not render the finalized quote PDF." };
  }
}
