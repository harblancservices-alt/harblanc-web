 ...
 * revenue pipeline (Estimate → Quote → BOL → Payment). This is the
 * customer-paperwork BOL rendered as an HTML/text email via Resend
 * (lib/email/bill-of-lading.ts) — a DIFFERENT system from Phase 6's
 * shared load-document signature compositing
 * (src/lib/domain/bol-signing.ts, PDF/image stamping via pdf-lib/sharp
 * onto uploaded load_documents rows). Same three letters, unrelated
 * tables, unrelated code paths — do not conflate the two.
 *
 * Shared by both /admin and /tms-v2, each of which adds only its own
 * app-specific behavior on top (demo-mode gate + revalidatePath targets
 * for /admin; revalidatePath targets only for /tms-v2, which has no
 * demo mode) — see the two wrapper files:
 * src/app/admin/(authed)/quotes/bol-actions.ts and
 * src/actions/tms-v2/pipeline.ts.
 *
 * Deliberately EXCLUDES resendBol — stays in admin's bol-actions.ts
 * unchanged; /tms-v2 doesn't call it (confirmed: pipeline.ts is
 * tms-v2's only entry point into the four quotes/*.ts admin files).
 *
 * Lifecycle:
 ...
 *          drafts).
 *
 * Send does NOT auto-advance lead_status — the dispatcher controls the
 * execution-status pipeline manually because real dispatch reality
 * doesn't always match a tidy linear sequence.
 *
...
// ─────────────────────────────────────────────────────────────────────────
//  1. Generate BOL Draft — entry point.
//
...
export async function buildBolPreview(
  formData: FormData,
): Promise<BolEmailPreview & { quoteRequestId: string }> {
  const bolId = s(formData.get("bol_id"));
  if (!bolId) throw new Error("Missing bol_id.");
  ...
  return { ...rendered, quoteRequestId: draft.quote_request_id };
}
