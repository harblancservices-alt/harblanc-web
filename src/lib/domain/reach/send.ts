 ...
 * app's Resend integration, reply-to the owner inbox so replies land in
 * Gmail. Shared by both /admin and /tms-v2, each of which adds only its own
 * demo-mode gate on top (tms-v2 has none — see src/actions/tms-v2/reach.ts
 * vs src/app/admin/(authed)/dispatch/reach/send-actions.ts). Nothing sends
 * until the operator taps in the UI. Each successful send is logged to
 * reach_sends, which powers the "reached Nd ago" suppression.
 *
 * No revalidatePath here by design (matches the original: neither send
 * function ever called it — the composer calls router.refresh() client-side
 * after a successful send instead).
 */
